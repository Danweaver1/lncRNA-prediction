transcript_TPMs <- read.table(file = "converted_TPM_subset.csv",sep = ',', header = TRUE,row.names=1)
TPM_cutoff <- 5

to_keep_full <- vector()
#keep features which have TPM cutoff or more in all 3 reps of at least one sample
...
###filter without HygB data
cut_TPMS <- transcript_TPMs[-25:-36]

to_keep_full <- vector()
#keep features which have TPM cutoff or more in all 3 reps of at least one sample
for (i in seq(1,ncol(cut_TPMS),by=3)){
  to_keep <- which(rowSums(cut_TPMS[,i:(i+2)]>=TPM_cutoff) >= 2)
  to_keep_full <- c(to_keep_full,to_keep)
}
final_to_keep2 <- unique(to_keep_full)
#extract TPM data for features to keep
filtered_data2 <- cut_TPMS[final_to_keep, ]
